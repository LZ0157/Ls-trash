...
def extract_coref_hint(text):
    """
    轻量级指代/实体提取（替代SGDCI中复杂的共指链矩阵）
    """
    doc = nlp(text)
    entities = [(ent.text, ent.label_) for ent in doc.ents]
    if entities:
        return f"检测到的实体/指代线索: {entities}"
    return None
...
def build_prompt(context, persona=None):
    ...
    for turn in context:
        if turn["role"] == "bot":
            hint = extract_coref_hint(turn["content"])
            if hint:
                struct_hint += f"针对回复'{turn['content'][:10]}...'的{hint}\n"

    if struct_hint:
        struct_hint = f"【系统预提取的结构化线索（指代与实体）】\n{struct_hint}\n"
    # =========================================================
    #
    # # 在构建 Prompt 时，自己写个简单的规则提取历史事实作为 KB
    # dynamic_kb = []
    # for turn in context[:-2]:  # 看最后一句之前的对话
    #     if turn["role"] == "bot":
    #         dynamic_kb.append(f"已知事实: {turn['content']}")
    #
    # kb_str = ""
    # if dynamic_kb:
    #     kb_str = "【对话动态知识库 (KB)】\n" + "\n".join(dynamic_kb) + "\n\n"
...
def extract_dependency_hint(text):
    """
    提取核心谓语和依存关系（替代SGDCI中复杂的AMR语义图）
    """
    doc = nlp(text)
    relations = []
    for token in doc:
        ...
        if token.pos_ == "VERB":
            ...
            if subj or obj:
                relations.append(f"动作[{token.text}] - 主语:{subj} - 宾语:{obj}")
    if relations:
        return f"局部语义结构(依存图): {relations}"
    return None
