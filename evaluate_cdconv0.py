...
DATA_FILE = "cdconv_mock.jsonl"  # 数据文件路径（如果是真实数据，改成真实的路径如 cdconv.txt）
...
def load_cdconv_data(file_path):
    """
    加载数据集。
    注意：如果你下载的是原版 cdconv.txt (可能是 TSV 格式)，
    你需要在这里修改解析逻辑。这里以标准的 JSONL 格式为例。
    """
    data = []
...
def run_evaluation():
    ...
    with open("eval_results_details0.json", "w", encoding="utf-8") as f:
